...
const currentMonthTotalAmount = computed<TransactionListDisplayTotalAmount | null>(() => {
    ...
    if (queryMonthlyData.value) {
        ...
        return {
            income: getDisplayCurrencyTotalAmounts(transactionData.totalAmountByCurrency.income),
            expense: getDisplayCurrencyTotalAmounts(transactionData.totalAmountByCurrency.expense)
        };
    } else {
...
function loadFullRangeTotalAmount(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        services.getAllTransactions({
            ...
            const totalAmount = calculateCurrencyTotalAmounts(Transaction.ofMulti(data.result), query.value.accountIds, defaultCurrency.value);
