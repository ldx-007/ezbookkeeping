...
**注：该功能已在代码审查阶段移除。** 原因：移动端选择自定义日期时，不同月份的交易已被归类到各自月份的折叠框中，每个月份已有自己的总额展示。额外在顶部加总额卡片属于冗余展示。

文件：`src/views/mobile/transactions/ListPage.vue`

原改动涉及以下内容，已在审查阶段清理：

1. 列表页顶部的总收入 / 总支出卡片（模板 + 样式）
2. `currentListTotalAmount` computed 属性
3. `mergeCurrencyTotalAmounts` / `getDisplayCurrencyTotalAmounts` 辅助函数
4. `TransactionSummaryDisplayTotalAmount` / `TransactionCurrencyAmount` 类型引用
...
### 8.3 ~~移动端总额区域后续可继续优化~~（已随卡片移除）

移动端顶部总额卡片已在审查阶段移除，此注意事项不再适用。
...
### 11.7 移除：移动端冗余的总额展示卡片

**问题：** 移动端自定义日期下交易已按月份归类到各自折叠框中，每个月份已有 `totalAmountByCurrency` 展示，顶部额外显示的汇总卡片属于冗余信息。

**处理：** 整体移除了移动端总额展示卡片，包括模板、CSS、`currentListTotalAmount` computed、`mergeCurrencyTotalAmounts` / `getDisplayCurrencyTotalAmounts` 辅助函数及相关类型引用。
